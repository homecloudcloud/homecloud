...
def get_hostname():
    try:
        ...
        if (check_service_status("tailscaled.service") == "Up"):
            ...
            # For x86/amd64/x64 systems, use hostname
            if arch in ['x86_64', 'amd64', 'x86']:
                hostname = socket.gethostname()
            else:
                # For ARM/Raspberry Pi, use existing IP logic
                ip_address = None
...
def get_system_timezone():
    ...
    # First check and update existing file if needed
    try:
        if os.path.exists(tzinfo_file):
            with open(tzinfo_file, 'r') as f:
                existing_timezone = f.read().strip()
                if existing_timezone == 'Asia/Calcutta':
                    with open(tzinfo_file, 'w') as f:
                        f.write('Asia/Kolkata')
    except Exception:
        pass
    ...
    try:
        ...
        # Convert Asia/Calcutta to Asia/Kolkata
        ...
        try:
            if os.path.exists(tzinfo_file):
                with open(tzinfo_file, 'r') as f:
                    ...
                    if timezone:
                        # Convert Asia/Calcutta to Asia/Kolkata for cached value too
                        if timezone == 'Asia/Calcutta':
                            timezone = 'Asia/Kolkata'
...
def restart_urbackup_server():
    try:
        subprocess.run(["systemctl", "restart", "urbackup.service"], check=False)
    except:
        pass
    return True
...
def update_smtp_config():
    try:
        ...
        scripts = [
            ...
            "/sbin/update-urbackup-smtp.py",
            "/sbin/update-linshare-smtp.py"
        ]
...
def update_photoprism_YAML(**kwargs): # updates YAML config 
    try:
        ...
        urbackup_yaml = f'/etc/urbackup/docker-compose.yaml'
        linshare_yaml = f'/etc/linshare/docker-compose.yaml'
        juicefs_yaml = f'/etc/juicefs/docker-compose.yaml'
        ...
        if (os.path.exists(duplicati_yaml)):
            print (f"duplicati - opening duplicati_yaml")
            ...
            for i, volume in enumerate(volumes):
                ...
                if i == 9: # first line in volumes is for internal storage home directory so we skip it
                    continue
                if i == 10: # first line in volumes is for internal storage home directory so we skip it
                    continue
                if i == 11: # first line in volumes is for internal storage home directory so we skip it
                    continue
                found = 0
        ...
        if (os.path.exists(urbackup_yaml)):
            with open(urbackup_yaml, 'r') as file1:
                urbackup_data = yaml.safe_load(file1)
            file1.close()
            hostname = get_hostname()
            if ('labels' not in urbackup_data['services']['urbackup']):                
                urbackup_data['services']['urbackup']['labels'] = []
                urbackup_data['services']['urbackup']['labels'].append('traefik.enable=true')
                urbackup_data['services']['urbackup']['labels'].append(f'traefik.http.routers.urbackup.rule=PathPrefix(`/`)')
                urbackup_data['services']['urbackup']['labels'].append('traefik.http.routers.urbackup.tls=true')
                urbackup_data['services']['urbackup']['labels'].append('traefik.http.services.urbackup.loadbalancer.server.port=55414')
                urbackup_data['services']['urbackup']['labels'].append('traefik.http.services.urbackup.loadbalancer.passhostheader=true')
                urbackup_data['services']['urbackup']['labels'].append('traefik.http.middlewares.urbackup-headers.headers.stsSeconds=31536000')
                urbackup_data['services']['urbackup']['labels'].append('traefik.http.routers.urbackup.middlewares=urbackup-headers')

            # Update PUID and PGID with admin user's uid and gid
            admin_user = pwd.getpwnam('admin')
            urbackup_data['services']['urbackup']['environment'][0] = f'PUID={admin_user.pw_uid}'
            urbackup_data['services']['urbackup']['environment'][1] = f'PGID={admin_user.pw_gid}'
            #print (f"urbackup - updating uid")
            ...
            if (timezone != None):
                tz_found = 0
                for i, item in enumerate(urbackup_data['services']['urbackup']['environment']):
                    if item.startswith('TZ'):
                        urbackup_data['services']['urbackup']['environment'][i] = f'TZ={timezone}'
                        tz_found = 1
                if tz_found == 0:
                    # Append with correct format without newline
                    urbackup_data['services']['urbackup']['environment'].append(f'TZ={timezone}')

            mountpoint_tba_urbackup = []
            mountpoints_urbackup=[]
            mountpoints_urbackup=getmountpoints()          
            change_in_disks = 0
            if (mountpoints_urbackup): # check if anything need to be added (if new disks inserted)
                for mount1 in mountpoints_urbackup:
                    found = 0
                    for volume in volumes:
                        if (mount1 in volume):
                            print(f"found:{mount} in {volume}")
                            ...
                            continue
                    if (found == 0):
                        mountpoint_tba_urbackup.append(mount1)
                        change_in_disks = 1
                if (mountpoint_tba_urbackup):
                    mount1 = ""
                    for mount1 in mountpoint_tba_urbackup:
                        model1=get_disk_model_from_mountpoint(mount1)
                        urbackup_data['services']['urbackup']['volumes'].append(f'{mount1}:/external-storage/{model1}-{mount1.replace("/srv/","")}')
            mountpoint_tba_urbackup = []
            ...
            for i, volume in enumerate(volumes):
                ...
                if i == 1: # first line in volumes is for internal storage dummy directory so we skip it - this is to make sure external disks links are not in family home directory
                    continue
                # Skip JuiceFS cloud backup volume
                if '/var/lib/jfsdata:/backups-cloud' in volume:
                    continue
                found = 0
                if (mountpoints_urbackup):
                    for mount1 in mountpoints:
                        if (mount1 in volume):
                            found = 1
                            continue
                if (found == 0): # need this volume string to be deleted from YAML
                    mountpoint_tba_urbackup.append(volume)
                    change_in_disks = 1

            if (len(mountpoint_tba_urbackup)>0):
                for i, volume in enumerate(mountpoint_tba_urbackup):
                    #print (f"volume is: {volume}")
                    for z, volume_to_delete in enumerate(urbackup_data['services']['urbackup']['volumes']):
                        if (volume == volume_to_delete ):
                            #print(f"found {volume} to be deleted")
                            del urbackup_data['services']['urbackup']['volumes'][z]

            
            yaml_content = yaml.safe_dump(urbackup_data , default_flow_style=False)
            safe_write_file(urbackup_yaml, yaml_content, mode="w", encoding="utf-8", lock_timeout=5.0, backup=True)

        if (os.path.exists(linshare_yaml)):
            with open(linshare_yaml, 'r') as file1:
                linshare_data = yaml.safe_load(file1)
            file1.close()
            if (timezone != None):
                tz_found = 0
                for i, item in enumerate(linshare_data['services']['backend']['environment']):
                    if item.startswith('TZ'):
                        linshare_data['services']['backend']['environment'][i] = f'TZ={timezone}'
                        tz_found = 1
                if tz_found == 0:
                    # Append with correct format without newline
                    linshare_data['services']['backend']['environment'].append(f'TZ={timezone}')

            hostname = get_hostname()
            i=0
            for i, item in enumerate(linshare_data['services']['ui-user']['environment']):
                if item.startswith('EXTERNAL_URL'):
                    linshare_data['services']['ui-user']['environment'][i] = f'EXTERNAL_URL=https://{hostname}:8301'
            i=0
            for i, item in enumerate(linshare_data['services']['ui-admin']['environment']):
                if item.startswith('EXTERNAL_URL'):
                    linshare_data['services']['ui-admin']['environment'][i] = f'EXTERNAL_URL=https://{hostname}:8301'
            i=0
            for i, item in enumerate(linshare_data['services']['ui-upload-request']['environment']):
                if item.startswith('EXTERNAL_URL'):
                    linshare_data['services']['ui-upload-request']['environment'][i] = f'EXTERNAL_URL=https://{hostname}:8301'
            

            if ('labels' not in linshare_data['services']['ui-user']):                
                linshare_data['services']['ui-user']['labels'] = []
                linshare_data['services']['ui-user']['labels'].append('traefik.enable=true')
                linshare_data['services']['ui-user']['labels'].append('traefik.http.routers.linshare.entrypoints=linshare')
                linshare_data['services']['ui-user']['labels'].append('traefik.http.routers.linshare.rule=PathPrefix(`/`)')
                linshare_data['services']['ui-user']['labels'].append('traefik.http.routers.linshare.tls=true')
                linshare_data['services']['ui-user']['labels'].append('traefik.http.services.linshare.loadbalancer.server.port=8300')
                linshare_data['services']['ui-user']['labels'].append('traefik.http.services.linshare.loadbalancer.passhostheader=true')
            #replace the value of EXTERNAL_URL variable
            

            if ('labels' not in linshare_data['services']['ui-admin']):                
                linshare_data['services']['ui-admin']['labels'] = []
                linshare_data['services']['ui-admin']['labels'].append('traefik.enable=true')
                linshare_data['services']['ui-admin']['labels'].append('traefik.http.routers.linshare.entrypoints=linshare')
                linshare_data['services']['ui-admin']['labels'].append('traefik.http.routers.linshare.rule=PathPrefix(`/admin`)')
                linshare_data['services']['ui-admin']['labels'].append('traefik.http.routers.linshare.tls=true')
                linshare_data['services']['ui-admin']['labels'].append('traefik.http.services.linshare.loadbalancer.server.port=8300')
                linshare_data['services']['ui-admin']['labels'].append('traefik.http.services.linshare.loadbalancer.passhostheader=true')
            ...
            yaml_content = yaml.safe_dump(linshare_data , default_flow_style=False)
            safe_write_file(linshare_yaml, yaml_content, mode="w", encoding="utf-8", lock_timeout=5.0, backup=True)


        if (os.path.exists(juicefs_yaml)):
            with open(juicefs_yaml, 'r') as file:
                juicefs_data = yaml.safe_load(file)
            file.close()
            timezone_found = 0
            for i, item in enumerate(juicefs_data['services']['juicefs']['environment']):
                if item.startswith('TZ'):
                    juicefs_data['services']['juicefs']['environment'][i] = (f'TZ={timezone}')
                    timezone_found=1
            if (timezone_found==0):
                juicefs_data['services']['juicefs']['environment'].append(f'TZ={timezone}')
            yaml_content = yaml.safe_dump(juicefs_data, default_flow_style=False)
            safe_write_file(juicefs_yaml, yaml_content, mode="w", encoding="utf-8", lock_timeout=5.0, backup=True)
...
def getappsendpoint():
    ...
    try:
        ...
        try:
            result = subprocess.run(
                ['omv-rpc', '-u', 'admin', 'Homecloud', 'getUrbackupServiceStatus'],
                capture_output=True, text=True, check=True
            )
            data = json.loads(result.stdout)
            if data.get('status') == 'Running' and 'hostname' in data:
            ...
            print(f"Error getting UrBackup status: {e}")
        
        try:
            result = subprocess.run(
                ['omv-rpc', '-u', 'admin', 'Homecloud', 'getLinshareServiceStatus'],
                capture_output=True, text=True, check=True
            )
            data = json.loads(result.stdout)
            if data.get('status') == 'Running' and 'hostname' in data:
            ...
            print(f"Error getting Linshare status: {e}")
...
def setup_urbackup_firewall():
    try:
        # Check if urbackup.service exists using systemctl list-unit-files
        check_service_cmd = ["systemctl", "list-unit-files", "urbackup.service"]
        service_result = subprocess.run(check_service_cmd, 
                                     capture_output=True, 
                                     text=True, 
                                     check=False)
        ...
        # Path to docker-compose file
        compose_file = "/etc/urbackup/docker-compose.yml"
        
        def get_urbackup_ports():
            try:
                if os.path.exists(compose_file):
                    with open(compose_file, 'r') as f:
                        compose_data = yaml.safe_load(f)
                    ...
                    # Get host port mappings from ports section
                    if ('services' in compose_data and 
                        'urbackup' in compose_data['services'] and 
                        'ports' in compose_data['services']['urbackup']):
                        
                        ports = compose_data['services']['urbackup']['ports']
                        
                        for port_mapping in ports:
                            # Extract host port and protocol
                            if '/' in port_mapping:
                                host_port = port_mapping.split(':')[0]
                                protocol = port_mapping.split('/')[-1] if '/' in port_mapping else 'tcp'
                            else:
                                host_port = port_mapping.split(':')[0]
                                protocol = 'tcp'
                            
                            port_list.append({'port': host_port.strip(), 'protocol': protocol, 'chain': 'INPUT'})
                    
                    # Get Traefik loadbalancer port from labels
                    if ('services' in compose_data and 
                        'urbackup' in compose_data['services'] and 
                        'labels' in compose_data['services']['urbackup']):
                        
                        labels = compose_data['services']['urbackup']['labels']
                        
                        for label in labels:
                            if 'traefik.http.services.urbackup.loadbalancer.server.port=' in label:
                                traefik_port = label.split('=')[-1]
                                port_list.append({'port': traefik_port.strip(), 'protocol': 'tcp', 'chain': 'DOCKER-USER'})
                                break
                    
                    return port_list
                return []
            except Exception as e:
                print(f"Error reading ports from docker-compose: {str(e)}")
                return []
        
        if "urbackup.service" in service_result.stdout:
            # Get ports from docker-compose
            ports = get_urbackup_ports()
            if not ports:
                print("Could not determine urbackup ports, using defaults")
                ports = [{'port': '55414', 'protocol': 'tcp'}, {'port': '55413', 'protocol': 'tcp'}, 
                        {'port': '55415', 'protocol': 'tcp'}, {'port': '35623', 'protocol': 'udp'}]
            
            for port_info in ports:
                port = port_info['port']
                protocol = port_info['protocol']
                chain = port_info['chain']
                
                # Define the rule components based on chain type
                if chain == 'DOCKER-USER':
                    # For Docker containers, restrict from non-Tailscale networks
                    rule_specs = [
                        "!", "-s", "0.0.0.0/0",
                        "-p", protocol,
                        "--dport", port,
                    ...
                    # For host services (INPUT chain), allow all traffic
                    rule_specs = [
                        "-s", "0.0.0.0/0",
                        "-p", protocol,
                        "--dport", port,
                ...
                # Check if the rule already exists to avoid duplicates
                check_rule_cmd = ["iptables", "-C", chain] + rule_specs
                ...
                if not rule_exists:
                    # Add the firewall rule
                    add_rule_cmd = ["iptables", "-I", chain, "1"] + rule_specs
                    subprocess.run(add_rule_cmd, check=True)
                
        else:
            # Service doesn't exist, remove any existing rules for urbackup
            possible_ports = [{'port': '55414', 'protocol': 'tcp', 'chain': 'INPUT'}, 
                             {'port': '55413', 'protocol': 'tcp', 'chain': 'INPUT'}, 
                             {'port': '55415', 'protocol': 'tcp', 'chain': 'INPUT'}, 
                             {'port': '35623', 'protocol': 'udp', 'chain': 'INPUT'},
                             {'port': '55414', 'protocol': 'tcp', 'chain': 'DOCKER-USER'}]
            
            # Also check docker-compose file for current ports
            current_ports = get_urbackup_ports()
            for port_info in current_ports:
                if port_info not in possible_ports:
                    possible_ports.append(port_info)
            ...
            for port_info in possible_ports:
                ...
                if rule_exists:
                    # Remove the firewall rule
                    remove_rule_cmd = ["iptables", "-D", chain] + rule_specs
                    subprocess.run(remove_rule_cmd, check=True)
                    print(f"Firewall rule removed for port {port}/{protocol} on {chain} as urbackup service does not exist")
...
def setup_linshare_firewall():
    try:
        compose_file = "/etc/linshare/docker-compose.yaml"
        ...
        def get_tomcat_port():
            try:
                if os.path.exists(compose_file):
                    ...
                    if ('services' in compose_data and 
                        'ui-user' in compose_data['services'] and 
                        'environment' in compose_data['services']['ui-user']):
                        
                        env = compose_data['services']['ui-user']['environment']
                        
                        # Handle both list and dict formats
                        if isinstance(env, list):
                            for item in env:
                                if 'TOMCAT_PORT=' in item:
                                    return item.split('=')[1].strip()
                        elif isinstance(env, dict):
                            return str(env.get('TOMCAT_PORT', '')).strip()
                return None
            except Exception as e:
                print(f"Error reading TOMCAT_PORT from docker-compose: {str(e)}")
                return None
        
        check_service_cmd = ["systemctl", "list-unit-files", "linshare.service"]
        service_result = subprocess.run(check_service_cmd, 
                                     capture_output=True, 
                                     text=True, 
                                     check=False)
        ...
        if not port:
            print("Could not determine linshare TOMCAT_PORT")
            return
        
        rule_specs = {
            'tcp': ["-s", "0.0.0.0/0", "-p", "tcp", "--dport", port, "-j", "REJECT"],
            'udp': ["-s", "0.0.0.0/0", "-p", "udp", "--dport", port, "-j", "REJECT"]
        }
        
        if "linshare.service" in service_result.stdout:
            for protocol in ['tcp', 'udp']:
                check_rule_cmd = ["iptables", "-C", "DOCKER-USER"] + rule_specs[protocol]
                rule_exists = subprocess.run(check_rule_cmd, capture_output=True, check=False).returncode == 0
            ...
            for protocol in ['tcp', 'udp']:
                ...
                if rule_exists:
                    ...
                    subprocess.run(remove_rule_cmd, check=True)
                    print(f"Firewall {protocol.upper()} rule removed for linshare as service does not exist")
...
@app.route('/setup_urbackup_firewall', methods=['POST'])
@omv_auth_required ###OMV authentication code
@limiter.limit("20 per minute", exempt_when=is_localhost)  # Exempt localhost from rate limiting
def invoke_setup_urbackup_firewall():
    try:
        setup_urbackup_firewall()
        return "True"
    except:
        return "False"

@app.route('/setup_linshare_firewall', methods=['POST'])
@omv_auth_required ###OMV authentication code
@limiter.limit("20 per minute", exempt_when=is_localhost)  # Exempt localhost from rate limiting
def invoke_setup_linshare_firewall():
    try:
        setup_linshare_firewall()
        return "True"
    except:
        return "False"
...
@app.route('/check_snapshot_capabilities', methods=['GET'])
@local_network_only
@limiter.limit("20 per minute", exempt_when=is_localhost)
def check_snapshot_capabilities():
    """Check if a directory path supports snapshots"""
    try:
        ...
        mount_point = result.stdout.split('\n')[1].split()[-1]
        device = result.stdout.split('\n')[1].split()[0]
        
        # Check if device is LVM
        lvm_check = subprocess.run(['lvs', device], capture_output=True, text=True)
        if lvm_check.returncode == 0:
            # Check free extents
            vg_name = lvm_check.stdout.split('\n')[1].split()[1]
            vg_info = subprocess.run(['vgs', '--noheadings', '-o', 'vg_free', vg_name], capture_output=True, text=True)
            if vg_info.returncode == 0 and vg_info.stdout.strip() != '0':
                return jsonify({'snapshot_capable': True}), 200
        
        # Check filesystem type and free space
        fs_check = subprocess.run(['stat', '-f', '-c', '%T %a %S', path], capture_output=True, text=True)
        if fs_check.returncode == 0:
            fs_type, free_blocks, block_size = fs_check.stdout.strip().split()
            free_space = int(free_blocks) * int(block_size)
            
            # Check if filesystem supports snapshots (btrfs, zfs)
            if fs_type in ['btrfs', 'zfs'] and free_space > 1024*1024*1024:  # 1GB minimum
                return jsonify({'snapshot_capable': True}), 200
        
        return jsonify({'snapshot_capable': False}), 200
        
    except Exception as e:
        return jsonify({'status': 'error', 'message': str(e)}), 500


def _remove_all_snapshots_internal():
    """Internal function to remove all snapshots without Flask decorators"""
    try:
        removed_count = 0
        
        # Remove LVM snapshots
        lvm_result = subprocess.run(['lvs', '--noheadings', '-o', 'lv_name,vg_name,lv_attr'], capture_output=True, text=True)
        if lvm_result.returncode == 0:
            for line in lvm_result.stdout.strip().split('\n'):
                if line.strip():
                    parts = line.strip().split()
                    if len(parts) >= 3:
                        lv_name, vg_name, lv_attr = parts[0], parts[1], parts[2]
                        if lv_attr.startswith('s') or (lv_attr.startswith('Swi') or lv_attr.startswith('swi')):
                            mount_result = subprocess.run(['findmnt', '-n', '-o', 'TARGET', f'/dev/{vg_name}/{lv_name}'], capture_output=True, text=True)
                            if mount_result.returncode == 0:
                                for mount_point in mount_result.stdout.strip().split('\n'):
                                    if mount_point.strip():
                                        subprocess.run(['umount', '-f', mount_point.strip()], check=False)
                            subprocess.run(['lvremove', '-f', f'/dev/{vg_name}/{lv_name}'], check=False)
                            removed_count += 1
        
        # Remove ZFS snapshots
        zfs_result = subprocess.run(['zfs', 'list', '-H', '-t', 'snapshot', '-o', 'name'], capture_output=True, text=True)
        if zfs_result.returncode == 0:
            for snapshot in zfs_result.stdout.strip().split('\n'):
                if snapshot.strip():
                    subprocess.run(['zfs', 'destroy', snapshot.strip()], check=False)
                    removed_count += 1
        return removed_count
    except Exception:
        return 0

@app.route('/remove_all_snapshots', methods=['POST'])
@local_network_only
@limiter.limit("5 per minute", exempt_when=is_localhost)
def remove_all_snapshots():
    """Remove all existing snapshots (LVM, Btrfs, ZFS)"""
    try:
        removed_count = _remove_all_snapshots_internal()
        return jsonify({'success': True, 'message': f'Removed {removed_count} snapshots'}), 200
    except Exception as e:
...
@limiter.limit("10 per minute", exempt_when=is_localhost)
def take_snapshot():
    """Take snapshot of directory and mount at specified mountpoint"""
    try:
        path = request.args.get('path')
        mountpoint = request.args.get('mountpoint')
        
        if not path or not mountpoint:
            return jsonify({'success': False, 'message': 'Path and mountpoint parameters required'}), 400
        
        # Check if mountpoint is already mounted
        mount_check = subprocess.run(['mountpoint', '-q', mountpoint], capture_output=True)
        if mount_check.returncode == 0:
            return jsonify({'success': True, 'message': 'Mountpoint already mounted'}), 200
        
        # Check snapshot capabilities first
        df_result = subprocess.run(['df', path], capture_output=True, text=True)
        if df_result.returncode != 0:
            return jsonify({'success': False, 'message': 'Invalid path'}), 400
        
        device = df_result.stdout.split('\n')[1].split()[0]
        
        # Check if LVM with free extents
        lvm_check = subprocess.run(['lvs', device], capture_output=True, text=True)
        if lvm_check.returncode == 0:
            vg_name = lvm_check.stdout.split('\n')[1].split()[1]
            lv_name = lvm_check.stdout.split('\n')[1].split()[0].split('/')[-1]
            
            # Check free extents and calculate snapshot size
            vg_info = subprocess.run(['vgs', '--noheadings', '-o', 'vg_free', vg_name], capture_output=True, text=True)
            if vg_info.returncode != 0 or vg_info.stdout.strip() == '0':
                return jsonify({'success': False, 'message': 'No free extents for snapshot'}), 400
            
            # Parse free space and calculate 40% of it
            free_space_str = vg_info.stdout.strip().lstrip('<').strip()
            # Extract numeric value and unit (e.g., "10.5g" -> 10.5, "g")
            import re
            match = re.match(r'([0-9.]+)([kmgtKMGT])', free_space_str)
            if match:
                size_val = float(match.group(1))
                unit = match.group(2).lower()
                
                # Calculate 40% of free space
                snapshot_size = size_val * 0.4
                
                # Format size with unit (keep same unit)
                snapshot_size_str = f"{snapshot_size:.1f}{unit}"
            else:
                # Fallback to 1G if parsing fails
                snapshot_size_str = "1G"
            
            # Create LVM snapshot with unique name
            import time
            timestamp = int(time.time())
            snapshot_name = f"{lv_name}_snapshot_{timestamp}"
            
            subprocess.run(['lvcreate', f'-L{snapshot_size_str}', '-s', '-n', snapshot_name, device], check=True)
            
            # Mount snapshot read-only
            subprocess.run(['mkdir', '-p', mountpoint], check=True)
            subprocess.run(['chown', 'admin:users', mountpoint], check=True)
            subprocess.run(['mount', '-o', 'ro', f'/dev/{vg_name}/{snapshot_name}', mountpoint], check=True)
            
            return jsonify({'success': True, 'message': 'LVM snapshot created and mounted'}), 200
        ...
        if fs_check.returncode == 0:
            ...
            if fs_type == 'btrfs' and free_space > 1024*1024*1024:
                # Create btrfs snapshot with unique name
                import time
                timestamp = int(time.time())
                snapshot_path = f"{path}_snapshot_{timestamp}"
                subprocess.run(['btrfs', 'subvolume', 'snapshot', '-r', path, snapshot_path], check=True)
                subprocess.run(['mkdir', '-p', mountpoint], check=True)
                subprocess.run(['chown', 'admin:users', mountpoint], check=True)
                subprocess.run(['mount', '--bind', snapshot_path, mountpoint], check=True)
                
                return jsonify({'success': True, 'message': 'Btrfs snapshot created and mounted'}), 200
            
            elif fs_type == 'zfs' and free_space > 1024*1024*1024:
                # Get ZFS dataset name
                zfs_info = subprocess.run(['zfs', 'list', '-H', '-o', 'name', path], capture_output=True, text=True)
                if zfs_info.returncode == 0:
                    dataset = zfs_info.stdout.strip()
                    import time
                    timestamp = int(time.time())
                    snapshot_name = f"{dataset}@snapshot_{timestamp}"
                    
                    # Create ZFS snapshot
                    subprocess.run(['zfs', 'snapshot', snapshot_name], check=True)
                    subprocess.run(['mkdir', '-p', mountpoint], check=True)
                    subprocess.run(['chown', 'admin:users', mountpoint], check=True)
                    subprocess.run(['mount', '-t', 'zfs', '-o', 'ro', snapshot_name, mountpoint], check=True)
                    
                    return jsonify({'success': True, 'message': 'ZFS snapshot created and mounted'}), 200
        
        return jsonify({'success': False, 'message': 'Snapshot not supported or insufficient space'}), 400
...
@app.route('/remove_snapshot', methods=['POST'])
@local_network_only
@limiter.limit("10 per minute", exempt_when=is_localhost)
def remove_snapshot():
    """Remove snapshot by unmounting and deleting it"""
    try:
        mountpoint = request.args.get('mountpoint')
        
        if not mountpoint:
            return jsonify({'success': False, 'message': 'Mountpoint parameter required'}), 400
        
        # Find all LVM snapshots
        lvs_result = subprocess.run(['lvs', '--noheadings', '-o', 'lv_name,vg_name'], capture_output=True, text=True)
        if lvs_result.returncode != 0:
            return jsonify({'success': False, 'message': 'Failed to list logical volumes'}), 500
        
        target_lv = None
        # First try to find snapshot by checking what's mounted at the mountpoint
        mount_result = subprocess.run(['findmnt', '-n', '-o', 'SOURCE', mountpoint], capture_output=True, text=True)
        if mount_result.returncode == 0:
            source = mount_result.stdout.strip()
            if '/dev/' in source and 'snapshot' in source:
                target_lv = source
        
        # If not found by mountpoint, search all snapshots
        if not target_lv:
            for line in lvs_result.stdout.strip().split('\n'):
                if line.strip():
                    lv_name, vg_name = line.strip().split()
                    if 'snapshot' in lv_name:
                        lv_path = f'/dev/{vg_name}/{lv_name}'
                        # Check if this snapshot is mounted at our mountpoint
                        check_mount = subprocess.run(['findmnt', '-n', '-o', 'TARGET', '-S', lv_path], capture_output=True, text=True)
                        if check_mount.returncode == 0 and mountpoint in check_mount.stdout:
                            target_lv = lv_path
                            break
        ...
        # Find all mount points for this snapshot
        mount_result = subprocess.run(['findmnt', '-n', '-o', 'TARGET', '-S', target_lv], capture_output=True, text=True)
        mount_points = []
        if mount_result.returncode == 0:
        ...
        for mp in mount_points:
            if mp.strip():
                try:
                    subprocess.run(['umount', '-f', mp.strip()], check=True)
                except subprocess.CalledProcessError:
        ...
        # Force remove the LVM snapshot
        try:
            subprocess.run(['lvremove', '-f', target_lv], check=True)
        except subprocess.CalledProcessError:
            # If lvremove fails, kill processes using the mountpoint and retry
            for mp in mount_points:
                if mp.strip():
                    subprocess.run(['fuser', '-km', mp.strip()], check=False)
            subprocess.run(['lvremove', '-f', target_lv], check=True)
        
        return jsonify({'success': True, 'message': 'Snapshot removed successfully'}), 200
...
def stop_container():
    try:
        ...
        valid_apps = ['paperless', 'immich', 'joplin', 'jellyfin', 'smb', 'vaultwarden', 'urbackup']
        ...
        if FIRST_START and not setup_in_progress:
            ...
            def run_setup():
                ...
                while (homecloud_ready == False):
                    ...
                    _remove_all_snapshots_internal()
                    setup_paperless_firewall()
                    setup_urbackup_firewall()
