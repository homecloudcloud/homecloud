...
def find_ethernet_interface():
    """
    Find the ethernet interface in the system - whether eth0, end0, or first working interface
    ...
    try:
        ...
        # If end0 or eth0 not found, find first working interface (for EC2)
        for line in result.stdout.split("\n"):
            if "routable" in line or "configured" in line:
                parts = line.split()
...
def get_connected_ssid():
    try:
        ...
        # Check if arch is x86/amd64/x64 and if iwgetid exists
        if arch in ['x86_64', 'amd64', 'x86']:
            # Check if iwgetid command exists
            iwgetid_check = subprocess.run(['which', 'iwgetid'], capture_output=True)
            if iwgetid_check.returncode != 0:
                return False
...
def get_hotspot_status(interface):
    try:
        ...
        # Check if arch is x86/amd64/x64 and if iwconfig exists
        if arch in ['x86_64', 'amd64', 'x86']:
            # Check if iwconfig command exists
            iwconfig_check = subprocess.run(['which', 'iwconfig'], capture_output=True)
...
def update_display():
    with update_display_lock:
        ...
        update_YAML() # this should take care of joplin, duplicati
        ...
        restart_service("duplicati.service")
        restart_service("urbackup.service")
...
if __name__ == "__main__":
    ...
    # Start the WiFi monitoring thread only if wlan0 exists
    try:
