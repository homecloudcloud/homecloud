...
def get_immich_api_endpoint() -> Optional[str]:
    """Get Immich API endpoint using omv-rpc command"""
    try:
...
def get_current_version() -> Optional[str]:
    """Get current version using Immich API endpoint"""
    try:
        api_endpoint = get_immich_api_endpoint()
        if not api_endpoint:
            return None
            
        response = requests.get(f"{api_endpoint}/server/version", 
                              timeout=10, 
...
def fetch_github_version() -> Optional[str]:
    ...
    try:
        # Run omv-rpc command
        cmd = ["omv-rpc", "-u", "admin", "Homecloud", "immich_get_latest_version"]
