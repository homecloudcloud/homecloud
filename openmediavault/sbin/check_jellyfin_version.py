...
def get_service_status() -> Dict:
    ...
    try:
        cmd = ["omv-rpc", "-u", "admin", "Homecloud", "getJellyfinServiceStatus"]
...
def get_deployed_version() -> Optional[str]:
    ...
    try:
        with open("/etc/jellyfin/docker-compose.yml", "r") as f:
            config = yaml.safe_load(f)
            image = config['services']['jellyfin']['image']
...
def get_available_version() -> Optional[str]:
    ...
    try:
        cmd = ["omv-rpc", "-u", "admin", "Homecloud", "jellyfin_get_latest_version"]
        returncode, stdout, stderr = run_command(cmd)
        if returncode == 0:
