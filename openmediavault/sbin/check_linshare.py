...
def check_directories():
    ...
    required_dirs = ['/var/lib/urbackup', '/etc/urbackup']
...
def get_linshare_status():
    """Determine urbackup status based on service and directories"""
    # First check if service exists by checking service status
    if not check_service_status('linshare.service'):
        return "Not deployed"
        
    dirs_exist = check_directories()
    service_running = check_service_status('linshare.service')
...
def main():
    ...
    hostname = f"https://{base_hostname}:8301"
