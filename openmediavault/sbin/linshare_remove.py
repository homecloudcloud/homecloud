...
def stop_and_remove_service():
    """Stop and remove the linshare service"""
    try:
        # Stop service
        print_status("Stopping linshare service...")
        subprocess.run(['systemctl', 'stop', 'linshare.service'], 
                      stderr=subprocess.DEVNULL)

        # Disable service
        print_status("Disabling linshare service...")
        subprocess.run(['systemctl', 'disable', 'linshare.service'],
                      stderr=subprocess.DEVNULL)
...
def get_env_variables():
    ...
    env_vars = {}
    try:
        return env_vars  # Urbackup doesn't use .env file
...
def remove_directories():
    ...
    try:
        file = Path('/etc/linshare/docker-compose.yaml')
        if file.exists():
...
def main():
    try:
        ...
        print_status("Linshare removal completed successfully")
        return 0
