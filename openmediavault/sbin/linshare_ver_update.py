...
def get_compose_version():
    ...
    try:
        with open('/etc/linshare/docker-compose.yaml ', 'r') as f:
            compose_data = yaml.safe_load(f)
            image = compose_data['services']['linshare']['image']
            if ':' in image:
...
def check_and_create_systemd_service():
    """Check if linshare service exists and create if it doesn't"""
    service_file = "/etc/systemd/system/linshare.service"
...
Restart=always
RestartSec=10
ExecStart=/usr/bin/docker compose -f /etc/linshare/docker-compose.yaml up 
ExecStop=/usr/bin/docker compose -f /etc/linshare/docker-compose.yaml down
...
def update_compose_version(version):
    ...
    try:
        ...
        compose_data['services']['database']['image'] = f'linagora/linshare-database:{version}'
        compose_data['services']['backend']['image'] = f'linagora/linshare-backend:{version}'
        compose_data['services']['ui-user']['image'] = f'linagora/linshare-ui-user:{version}'
        compose_data['services']['ui-admin']['image'] = f'linagora/linshare-ui-admin:{version}'
        compose_data['services']['init']['image'] = f'linagora/linshare-init:{version}'


        with open('/etc/linshare/docker-compose.yaml', 'w') as f:
...
def run_docker_pull():
    ...
    try:
        process = subprocess.Popen(
            "cd /etc/linshare && docker compose -f /etc/linshare/docker-compose.yaml pull",
            shell=True,
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Deploy or update Linshare')
    ...
    try:
        ...
        # it should exit if it is not x86,x86_64 or amd64

        if arch not in ['x86_64', 'amd64', 'x86']:
            print_status("Linshare does not work on Raspberry PI devices.", error=True)
            sys.exit(0)
        
        # Check deployment status
        print_status("Checking deployment status...")
        result = subprocess.run(['omv-rpc', '-u', 'admin', 'Homecloud', 'getLinshareServiceStatus'],
                              capture_output=True, text=True, check=True)
        ...
        if is_fresh:
            ...
            # New deployment
            print_status("Creating /etc/linshare directory...")
            os.makedirs('/etc/linshare', exist_ok=True)
            ...
            os.makedirs(f'{mount_point}/linshare', exist_ok=True)
            #os.chmod(f'{mount_point}/urbackup/backups', 0o777)
            subprocess.run(['chown', '-R', 'admin:users', f'{mount_point}/linshare'], check=True)
            
            # Remove existing /var/lib/duplicati if it exists
            if os.path.exists('/var/lib/linshare'):
                if os.path.islink('/var/lib/linshare'):
                    os.unlink('/var/lib/linshare')
                else:
                    shutil.rmtree('/var/lib/linshare')
            
            # Create symlink
            os.symlink(f'{mount_point}/linshare', '/var/lib/linshare')
            
            # Copy conf and ssl directories if they don't exist
            for dir_name in ['conf', 'ssl']:
                src_dir = f'/opt/homecloud/linshare/{dir_name}'
                dst_dir = f'/var/lib/linshare/{dir_name}'
            ...
            source_file='/etc/homecloud/docker-compose-linshare.yml'
            shutil.copy2(source_file, '/etc/linshare/docker-compose.yaml')
            print_status(f"Copied {source_file} to /etc/linshare/docker-compose.yaml")
            ...
            version_cmd = "omv-rpc -u admin 'Homecloud' 'linshare_check_version'"
            ...
            # Stop service
            print_status("Stopping Linshare service...")
            subprocess.run(['systemctl', 'stop', 'linshare.service'], check=True)
            ...
            for file in ['docker-compose.yaml']:
                if os.path.exists(f'/etc/linshare/{file}'):
                    shutil.copy2(f'/etc/linshare/{file}', f'/etc/linshare/{file}.{current_version}')
        ...
        try:
            response = requests.post(
                "https://localhost:5000/setup_linshare_firewall",
                verify=False,
                timeout=30
            )
        ...
        print_status("Enabling and starting Linshare service...")
        try:
            # Enable the service
            subprocess.run(['systemctl', 'enable', 'linshare.service'], check=False)
            # Unmask the service
            subprocess.run(['systemctl', 'unmask', 'linshare.service'], check=False)
            # Start the service
            subprocess.run(['systemctl', 'start', 'linshare.service'], check=True)
        except Exception as e:
