...
get_status() {
    local status
    if systemctl is-active --quiet juicefs.service; then
        status="running"
    else
        status="stopped"
    fi
...
wait_for_startup() {
    ...
    while true; do
        if journalctl -u juicefs.service -n 50 --no-pager | grep -q "urbackupfs is ready at /mnt/jfs"; then
            return 0
        fi
...
# Restart the service
systemctl restart juicefs.service
