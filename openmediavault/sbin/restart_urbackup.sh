...
get_status() {
    local status
    if systemctl is-active --quiet urbackup.service; then
        status="running"
    else
        status="stopped"
    fi
...
wait_for_startup() {
    ...
    while true; do
        if journalctl -u urbackup.service -n 50 --no-pager | grep -q "Server started up successfully"; then
            return 0
        fi
...
# Restart the service
systemctl restart urbackup.service
