...
def main():
    parser = argparse.ArgumentParser(description='Update Linshare SMTP configuration', add_help=False)
    ...
    env_file = "/etc/linshare/docker-compose.yaml"
    ...
    if args.state == "disabled":
        ...
        remove_env_var(compose_data, "SMTP_HOST")
        #remove_env_var(compose_data, "MAIL_FROM")
        #remove_env_var(compose_data, "MAIL_TO")
        remove_env_var(compose_data, "SMTP_USER")
        remove_env_var(compose_data, "SMTP_PASSWORD")
        remove_env_var(compose_data, "SMTP_PORT")
        remove_env_var(compose_data, "MAIL_PORT")
        remove_env_var(compose_data, "SMTP_SECURITY")
        remove_env_var(compose_data, "SMTP_START_TLS_ENABLE")
        remove_env_var(compose_data, "SMTP_TLS")
        ...
        update_env_var(compose_data, "SMTP_USER", args.smtp_user)
        update_env_var(compose_data, "SMTP_PASSWORD", args.smtp_password)
        update_env_var(compose_data, "SMTP_HOST", args.smtp_host)
        update_env_var(compose_data, "MAIL_PORT", args.smtp_port)
        update_env_var(compose_data, "SMTP_PORT", args.smtp_port)
        update_env_var(compose_data, "SMTP_SECURITY", "starttls")
        update_env_var(compose_data, "SMTP_START_TLS_ENABLE", "true")
        update_env_var(compose_data, "SMTP_TLS", "true")
