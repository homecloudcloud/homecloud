...
if [ "$state" = "disabled" ]; then
    ...
    update_env_var "SMTP_HOST" "$smtp_host" "$env_file"
    update_env_var "SMTP_FROM" "$from_email" "$env_file"
    update_env_var "SMTP_USER" "$smtp_user" "$env_file"
    update_env_var "SMTP_TO" "$admin_email" "$env_file"
    update_env_var "ADMIN_EMAIL" "$admin_email" "$env_file"
    update_env_var "MAILER_SECURITY" "starttls" "$env_file"
    update_env_var "SMTP_PASS" "$smtp_password" "$env_file"
fi
