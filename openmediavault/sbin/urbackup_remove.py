...
def stop_and_remove_service():
    ...
    try:
        ...
        subprocess.run(['systemctl', 'stop', 'urbackup.service'], 
                      stderr=subprocess.DEVNULL)

        # Disable service
        print_status("Disabling urbackup service...")
        subprocess.run(['systemctl', 'disable', 'urbackup.service'],
                      stderr=subprocess.DEVNULL)
...
def get_docker_images():
    ...
    images = set()
    try:
        compose_file = "/etc/urbackup/docker-compose.yaml"
...
def remove_directories():
    ...
    try:
        file = Path('/etc/urbackup/docker-compose.yaml')
        if file.exists():
...
def main():
    try:
        ...
        print_status("UrBackup removal completed successfully")
        return 0
