...
export class DatatablePageComponent extends AbstractPageComponent<DatatablePageConfig> {
  ...
  onDeleteActionClick(action: DatatableAction) {
    ...
    console.log(this.config.subTitle);
    ...
    if (isFormatable(this.config.rowEnumFmt)) {
      ...
      if(this.config.columns[0].name == 'Name' &&  this.config.columns[3].name == 'Email' &&  this.config.columns[4].name == 'Groups'){
        message = format(
          gettext(
            'Warning: Deleting user will also delete the corresponding user data including photos, files any backups etc.\n Do you really want to delete the selected Users(s) <strong>{{ items | join(", ") }}</strong>?'
          ),
          { items }
