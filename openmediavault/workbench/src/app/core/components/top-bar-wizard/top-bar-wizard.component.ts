...
*/
@Component({
  selector: 'omv-top-bar-wizard',
  templateUrl: './top-bar-wizard.component.html',
  styleUrls: ['./top-bar-wizard.component.scss']
})
export class TopBarWizardComponent {
  @Output()
...
*/
 ...
 /* onReboot(): void {
    this.showDialog(
      gettext('Restart'),
      gettext('Do you really want to reboot Homecloud? All users/apps would get disconnected?'),
      'confirmation-critical',
      () => {
        this.rpcService.request('System', 'reboot', { delay: 0 }).subscribe(() => {
          this.blockUiService.start(
            translate(gettext('Homecloud will reboot now. This may take some time ...'))
          );
