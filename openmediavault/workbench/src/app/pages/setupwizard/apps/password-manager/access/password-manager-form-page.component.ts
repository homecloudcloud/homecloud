      ...
      this.htmlContent = `
        <div>
          ...
          <div class="status-message ${this.vaultWardenStatus !== 'Running' ? '' : 'hidden'}">
            ...
            <span class="status-not-running-message ${this.vaultWardenStatus !== 'Running' && this.vaultWardenStatus !== 'Not deployed' ? '' : 'hidden'}" >App is not running currently. It may take a while for the app to start. Refresh the page to check status.</span>
          </div>
