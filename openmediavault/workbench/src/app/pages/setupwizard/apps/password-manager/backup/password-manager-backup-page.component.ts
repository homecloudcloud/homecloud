...
@Component({
  ...
  template: `
  <omv-intuition-form-page id="password-manager-backup-form1" [config]="this.config"></omv-intuition-form-page>
...
export class AppsPasswordManagerBackupComponent extends BaseFormPageComponent {
  ...
  public config: FormPageConfig = {
    request: {
      service: 'Homecloud',
      get: {
        method: 'get_backup_size_vaultwarden'
    ...
    fields: [
      ...
      {
        type: 'paragraph',
        title: gettext('Backup your Vaultwarden data to external USB disks plugged in to Homecloud')
      },
      {
        type: 'paragraph',
        title: gettext('Insert image vaultwarden-backup-1.png')
      },
      {
        type: 'textInput',
        name: 'total_gb',
        label: gettext('Estimated Vaultwarden backup size in GB'),
    ...
    actions: [
      {
        ...
        enabledConstraints: {
          ...
          message: gettext(
            'Backup will be stored in folder named vaultwarden_backup_timestamp in selected backup disk. Do you want to continue?'
          )
        },
        execute: {
          type: 'taskDialog',
          taskDialog: {
            config: {
              ...
              request: {
                service: 'Homecloud',
                method: 'vaultwarden_backup_execute',
                params:{
  ...
  ngOnInit(): void {
    this.rpcService.request('Homecloud', 'get_backup_size_vaultwarden', {}).subscribe((data: any) => {
