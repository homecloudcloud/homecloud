...
@Component({
  selector:'omv-cloudbackup-update-page', //Home cloud changes
  template: `<div id="cloudbackup-update-form1">
                <div class="omv-form-paragraph" [innerHTML]="safeHtmlContent"></div>
              </div>
              <omv-intuition-form-page id="cloudbackup-update-form2"[config]="this.config"></omv-intuition-form-page>
              <div id="cloudbackup-update-form3">
                <div class="omv-form-paragraph" [innerHTML]="safeHtmlContent1"></div>
              </div>
             <omv-intuition-form-page id="cloudbackup-update-form4"[config]="this.config2"></omv-intuition-form-page>`,
  styleUrls: ['./cloudbackup-update-form-page.component.scss'],
  encapsulation: ViewEncapsulation.None  // This will disable view encapsulation
})


export class AppsCloudbackupUpdateFormPageComponent extends BaseFormPageComponent {
  ...
  private htmlContent= `
            <div class="update-section">
              ...
              <p>
              📋 Before updating, please <a href="https://github.com/duplicati/duplicati/" class="plainLink" target="_blank">review the release notes</a> for important information.
              </p>
            </div>
  ...
  public config: FormPageConfig = {
    ...
    fields: [
      ...
      
      {
        type: 'textInput',
        name: 'status',
        label: gettext('New updates for Duplicati backend service'),
        hint: gettext(''),
        value: '',
        readonly: true
      },
      ...
      
      {
        ...
        hint: gettext('By checking this box, you agree to update Duplicati to selected version'),
        value: '',
        readonly: false
      }
    ],
    ...
    fields: [
      {
        type: 'textInput',
        name: 'status',
        label: gettext('Auto updates for Duplicati backend'),
        hint: gettext(''),
        value: '',
        readonly: true
      }
  ...
  ngOnInit():void{
    this.rpcService.request('Homecloud', 'appAutoUpdates',{
      ...
      this.buttonText = response.status === 'enabled' ? 'Disable Auto updates for Duplicati backend' : 'Enable Auto updates for Duplicati backend';
  ...
  fetchStatusAndUpdateFields(): void {
    this.rpcService.request('Homecloud', 'duplicati_check_version').subscribe(response => {
  ...
  updateFieldColors(status:string):void{
    const element = document.querySelector('omv-cloudbackup-update-page omv-form-text-input:nth-of-type(1) .mat-form-field input');
    if(element){
  ...
  updateFieldVisibility(status:string):void{
    const checkbox=document.querySelector('omv-cloudbackup-update-page omv-form-checkbox');
    const updateButton = document.querySelector('omv-cloudbackup-update-page omv-submit-button button');
