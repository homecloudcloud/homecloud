  ...
  public config: DatatablePageConfig = {
    ...
    hints: [
      {
        type: 'info',
        text: gettext(
          'Use these settings to give access to users.'
        )
      }
    ],
    selectionType: 'none',
    columns: [
      { name: gettext('User'), prop: 'name', flexGrow: 2, sortable: true },
      {
        name: gettext('Type'),
        hidden: true,
        prop: 'type',
    ...
    store: {
      proxy: {
        service: 'Homecloud',
        get: {
          method: 'getPrivilegesFilterGroups',
          params: {
    ...
    buttons: [
      {
        template: 'cancel',
        url: '/startconfiguration/apps/drive/shares'
      },
      {
        text: gettext('Save'),
  ...
  onSave() {
    ...
    this.rpcService
      ...
      .subscribe(() => {
        ...
        this.router.navigate(['/startconfiguration/apps/drive/shares']);
      });
  }
}
