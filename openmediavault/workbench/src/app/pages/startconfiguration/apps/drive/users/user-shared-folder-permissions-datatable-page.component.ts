  ...
  public config: DatatablePageConfig = {
    ...
    hints: [
      {
        type: 'info',
        text: gettext(
          'These settings are used to configure the access rights for the user "{{ _routeParams.name }}".'
        )
      }
    ],
    selectionType: 'none',
    columns: [
      { name: gettext('USB disk/Shared folder'), prop: 'name', flexGrow: 1, sortable: true },
      {
    ...
    store: {
      proxy: {
        service: 'Homecloud',
        get: {
          method: 'getPrivilegesByRoleFilterHomedirs',
          params: {
            role: 'user',
  ...
  onSave() {
    ...
    this.rpcService
      ...
      .subscribe(() => {
        ...
        this.router.navigate(['/startconfiguration/apps/drive/users']);
      });
  }
}
