...
@Component({
  selector:'omv-joplin-update-page', //Home cloud changes
  template: `<omv-intuition-form-page id="joplin-update-form"[config]="this.config"></omv-intuition-form-page>
             <omv-intuition-form-page id="joplin-update-form"[config]="this.config2"></omv-intuition-form-page>`,
  styleUrls: ['./joplin-update-form-page.component.scss'],
  encapsulation: ViewEncapsulation.None  // This will disable view encapsulation
})


export class AppsJoplinUpdateFormPageComponent extends BaseFormPageComponent {
  private statustosend:string='';
  private buttonConfig:FormPageButtonConfig= {
      text: 'Enable/Disable Auto updates for Joplin backend',
      disabled: false,
      submit: true,
  ...
  public config: FormPageConfig = {
    ...
    fields: [
      ...
      {
        type: 'paragraph',
        title: gettext('Joplin community releases update to backend service and mobile app. Mobile app updates are pushed to your phone via app store or playstore. Computer apps need to be manually updated')
      },
      ...
      {
        type: 'paragraph',
        title: gettext('Take Joplin backup before proceeding further. Go to Notes -> backup page for taking backup')
      },

      {
        type: 'textInput',
        name: 'status',
        label: gettext('New updates for Joplin backend service'),
        hint: gettext(''),
        value: '',
        readonly: true
      },
      ...
      },
      {
        ...
        hint: gettext('Joplin backend service latest version available'),
        value: '',
        readonly: true
      },
      {
        ...
        hint: gettext('By checking this box, you agree to update Joplin to latest version'),
        value: '',
        readonly: false
      }
    ],
    ...
    fields: [
      {
        type: 'textInput',
        name: 'status',
        label: gettext('Auto updates for Joplin backend'),
        hint: gettext(''),
        value: '',
        readonly: true
      }
  ...
  ngAfterViewInit(): void {
    ...
    setTimeout(() => {
      ...
      // Select all paragraph elements (assuming they are rendered as `photos-update-form omv-form-paragraph` elements)
        const paragraphs = document.querySelectorAll('#joplin-update-form .omv-form-paragraph');
  ...
  fetchStatusAndUpdateFields(): void {
    this.rpcService.request('Homecloud', 'joplin_check_version').subscribe(response => {
  ...
  updateFieldColors(status:string):void{
    const element = document.querySelector('omv-joplin-update-page omv-form-text-input:nth-of-type(1) .mat-form-field input');
    if(element){
  ...
  updateFieldVisibility(status:string):void{
    const checkbox=document.querySelector('omv-joplin-update-page omv-form-checkbox mat-checkbox');
    const updateButton = document.querySelector('omv-joplin-update-page omv-submit-button button');
