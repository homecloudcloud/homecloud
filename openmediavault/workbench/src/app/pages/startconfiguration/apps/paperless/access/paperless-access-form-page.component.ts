      ...
      this.htmlContent = `
        <div>
          ...
          <div class="status-message ${this.paperlessStatus !== 'Running' ? '' : 'hidden'}">
            <span class="status-deploy-message ${this.paperlessStatus !== 'Not deployed' ? 'hidden' : ''}" >App is not deployed. Go to <a class="plainLink" href="#/startconfiguration/apps/paperless">Paperless main page</a> to deploy the app. </span>
            <span class="status-not-running-message ${this.paperlessStatus !== 'Running' && this.paperlessStatus !== 'Not deployed' ? '' : 'hidden'}" >App is not running currently. Go to <a class="plainLink" href="#/startconfiguration/apps/paperless/restart">Paperless status page</a> to check the status or restart the app. </span>
          </div>
  ...
  `;
    ...
    this.rpcService.request('Homecloud', 'getFunnelStatus', { appname: 'paperless-ngx' }).subscribe(funnelResponse => {
      ...
      if (internetSharingStatus === 'Enabled') {
        ...
        if (internetSharingStatus === 'VPN down'){
          this.config2.buttons = []; //No buttons
        }
        else{ 
