...
@Component({
  selector:'omv-urbackup-access-page', //Home cloud changes
  template: `
  <div id="urbackup-access-form1">
    <div class="omv-form-paragraph" [innerHTML]="safeHtmlContent"></div>
  </div>
  <div id="urbackup-access-form2">
    <div class="omv-form-paragraph" [innerHTML]="safeHtmlContent1"></div>
  </div>
  `,
  styleUrls: ['./urbackup-access-form-page.component.scss'],
  encapsulation: ViewEncapsulation.None  // This will disable view encapsulation
})



export class AppsUrbackupAccessComponent extends BaseFormPageComponent {
    ...
    this.rpcService.request('Homecloud', 'getUrbackupServiceStatus').subscribe(response => {
      ...
      this.htmlContent = `
        <div>
          ...
          <div class="access-info ${this.urbackupStatus !== 'Running' ? 'hidden' : ''}">

            <h3>🌐 Access via Web</h3>
            <p>
              <p>
                <strong>Used for configuring backup</strong> server:
                </p>
              Simply click the button below: <br>
              <a class="app-btn ${this.urbackupStatus !== 'Running' ? 'disabled-btn' : ''}" href="${this.hostname}" target="_blank">Access Urbackup</a>
            </p>
          </div>
      </div>
      `;
