...
@Component({
  selector:'omv-urbackup-update-page',
  template: `<div id="urbackup-update-form1">
                <div class="omv-form-paragraph" [innerHTML]="safeHtmlContent"></div>
              </div>
              <omv-intuition-form-page id="urbackup-update-form2"[config]="this.config"></omv-intuition-form-page>
              <div id="urbackup-update-form3">
                <div class="omv-form-paragraph" [innerHTML]="safeHtmlContent1"></div>
              </div>
             <omv-intuition-form-page id="urbackup-update-form4"[config]="this.config2"></omv-intuition-form-page>`,
  styleUrls: ['./urbackup-update-form-page.component.scss'],
  encapsulation: ViewEncapsulation.None
})


export class AppsUrbackupUpdateFormPageComponent extends BaseFormPageComponent {
  ...
  private htmlContent= `
            <div class="update-section">
              ...
              <p>
              📋 Before updating, please <a href="https://github.com/uroni/urbackup_backend" class="plainLink" target="_blank">review the release notes</a> for important information.
              </p>
            </div>
  ...
  public config: FormPageConfig = {
    ...
    fields: [
      ...
      
      {
        type: 'textInput',
        name: 'status',
        label: gettext('New updates for UrBackup backend service'),
        hint: gettext(''),
        value: '',
        readonly: true
      },
      ...
      
      {
        ...
        hint: gettext('By checking this box, you agree to update UrBackup to selected version'),
        value: '',
        readonly: false
      }
    ],
    ...
    fields: [
      {
        type: 'textInput',
        name: 'status',
        label: gettext('Auto updates for UrBackup backend'),
        hint: gettext(''),
        value: '',
        readonly: true
      }
  ...
  ngOnInit():void{
    this.rpcService.request('Homecloud', 'appAutoUpdates',{
      ...
      this.buttonText = response.status === 'enabled' ? 'Disable Auto updates for UrBackup backend' : 'Enable Auto updates for UrBackup backend';
  ...
  fetchStatusAndUpdateFields(): void {
    this.rpcService.request('Homecloud', 'urbackup_check_version').subscribe(response => {
  ...
  updateFieldColors(status:string):void{
    const element = document.querySelector('omv-urbackup-update-page omv-form-text-input:nth-of-type(1) .mat-form-field input');
    if(element){
  ...
  updateFieldVisibility(status:string):void{
    const checkbox=document.querySelector('omv-urbackup-update-page omv-form-checkbox');
    const updateButton = document.querySelector('omv-urbackup-update-page omv-submit-button button');
